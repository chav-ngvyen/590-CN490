...
GAMMA_L2	=	0.01	#ccL2 REGULARIZATION CONSTANT
...
def minimizer(f,xi):
    ...
    while(iteration<=max_iter):
        ...
        if(iteration%10==0):
            ...
            print("MSE: ", MSE_T, "     ", "Deg Freedom: ", df) 
...
#------------------------
# NEURAL NET EVALUTION
#------------------------
def NN_eval(x,p):
    out = np.matmul(x,p[0].T) + p[1].T

    even = get_even_numbers(range(len(p)))

    # Skip the input layer
    for j in even[1:]:
        if(activation == "TANH"):
            out = tanh(out)
        out = np.matmul(out,p[j].T) + p[j+1].T
    return out
