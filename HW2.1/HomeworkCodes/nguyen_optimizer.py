...
iteration=0; iterations=[]; loss_list = []

def new_loss(p,xbatch,ybatch):
    # global iterations, loss_train, loss_val, iteration
    global iterations, iteration, lost_list

	#TRAINING LOSS
    ypredict=model(xbatch,p) #model predictions for given parameterization p
    my_loss=(np.mean((ypredict-ybatch)**2.0))  #MSE
...
res = minimize(new_loss, po, args=(xt, yt),  method=OPT_ALGO, tol=1e-15);  popt=res.x
...
if(IPLOT):
	fig, ax = plt.subplots()
	ax.plot(iterations, loss_list, 'o', label='Training loss')
